...
# setting up parameters expected from arguments
# temperature
if sys.argv[1] == 'temp':
    T_celsius = float(sys.argv[2])
    T = float(sys.argv[2]) + 273.15 # convert the input Celsius temp to Kelvin
else:
    T_celsius = 65
    T = 65 + 273.15
...
if sys.argv[1] == 'hcn':
    HCN_mass_influx_ox = float(sys.argv[2])
    HCN_mass_influx_red = float(sys.argv[3])
else:
    HCN_mass_influx_ox = 3.42599458e-12
    HCN_mass_influx_red = 1.25776186e-07
if sys.argv[1] == 'h2co':
    H2CO_mass_influx_ox = float(sys.argv[2])
    H2CO_mass_influx_red = float(sys.argv[3])
else:
...
def calc_m_dot_red(spec, mu_val, mu_val_hcn):
    ''' Calculates the mass increase due to the incoming mass flux of rain out of HCN and formaldehyde
        in the reducing model'''
    if spec == 'HCN':
        return HCN_mass_influx_red * 4 * math.pi * R_plus**2
    elif spec == 'formaldehyde':
        return H2CO_mass_influx_red * 4 *math.pi * R_plus**2
    else:
        return HCN_mass_influx_red * 4 * math.pi * R_plus**2 * mu_val/mu_val_hcn
    
def calc_m_dot_ox(spec, mu_val, mu_val_hcn):
    ''' Calculates the mass increase due to the incoming mass flux of rain out of HCN and formaldehyde
        in the oxidising model'''
    if spec == 'HCN':
        return HCN_mass_influx_ox * 4 * math.pi * R_plus**2
    elif spec == 'formaldehyde':
        return H2CO_mass_influx_ox * 4 *math.pi * R_plus**2
    else:
        return HCN_mass_influx_ox * 4 * math.pi * R_plus**2 * mu_val/mu_val_hcn
