...
k_65 = [0., 10**(-5902/T_65 + 8.15), 10**(-6330/T_65 + 9.40), 10**(-7649/T_65 + 11.76), 10**(-5620/T_65 + 8.69), 10**(-7709/T_65 + 11.24), 0., 0., 0., 10**(-6230/T_65 + 9.42), 10**(-5270/T_65 + 7.95)]
...
def calc_m_dot(influx, spec, mu_val, mu_val_hcn):
    if spec == 'HCN':
        return influx * 4 * math.pi * R_plus**2
    else:
        return influx * 4 * math.pi * R_plus**2 * mu_val/mu_val_hcn

# initialising dict in dict to store the all the info
constants_and_rates = {}
for spec,mu_spec,rho_spec,c_min,c_max,k in zip(species,MU,rho_species,c_05_min,c_05_max,k_65):
    constants_and_rates[spec] = {'mu': mu_spec,
                       ...
                       'm_dot_red': calc_m_dot(HCN_mass_influx_red, spec, mu_spec, MU[0]),
                       'm_dot_ox': calc_m_dot(HCN_mass_influx_ox, spec, mu_spec, MU[0]),
                       'k': k}
...
values = {'IDN':{}, 'COL':{}}
names = ['L_65_IDP', 'L_65_Met', 'm_65_IDP', 'm_65_Met', 'm_65_IDP_A', 'm_65_Met_A', 'C_65_IDP', 'C_65_Met'] +\
['m_65_' + spec + '_red' for spec in species] + ['m_65_' + spec + '_ox' for spec in species] + ['C_65_' + spec + '_red' for spec in species] + ['C_65_' + spec + '_ox' for spec in species] +\
['m_65_' + spec + '_haze05_min' for spec in species] + ['m_65_' + spec + '_haze05_max' for spec in species] + ['C_65_' + spec + '_haze05_min' for spec in species] + ['C_65_' + spec + '_haze05_max' for spec in species]
#m_hcn_and_biomolecules_red = ['m_65_' + spec + '_red' for spec in species]
#m_hcn_and_biomolecules_ox = ['m_65_' + spec + '_ox' for spec in species]
#C_hcn_and_biomolecules_red = ['C_65_' + spec + '_red' for spec in species]
#C_hcn_and_biomolecules_ox = ['C_65_' + spec + '_ox' for spec in species]
#m_haze05_min = ['m_65_' + spec + '_haze05_min' for spec in species]
#m_haze05_max = ['C_65_' + spec + '_haze05_max' for spec in species]
for prec_mod in values:
    for name in names:
        values[prec_mod][name] = np.zeros(shape=nt)
    values[prec_mod]['L_65_IDP'][0] = r_p - min_water
    values[prec_mod]['L_65_Met'][0] = r_p - min_water
    values[prec_mod]['m_65_IDP'][0] = math.pi*rho_w*r_p**2*(min_water) #og was r_p-(r_p-min_water)....
    values[prec_mod]['m_65_Met'][0] = math.pi*rho_w*r_p**2*(min_water)
...
def water_decrease(impactor, model, iteration):
    ...
    return E_65*delta_t + values[model]['L_65_'+impactor][iteration] - precipitation(model, iteration)
...
def water_mass(model, impactor, iteration):
    ...
    return math.pi*rho_w*r_p**2*(r_p-values[model]['L_65_'+impactor][iteration])
...
for n in range(0,nt-1):
    ...
    for prec_mod in values: # cycle all precipitation models (IDN and COL)
        ...
        for impact in ['Met', 'IDP']:
            values[prec_mod]['L_65_'+impact][n+1] = water_decrease(impact, prec_mod, n)
            if (values[prec_mod]['L_65_'+impact][n+1] < 0):
                values[prec_mod]['L_65_'+impact][n+1] = 0
            if (values[prec_mod]['L_65_'+impact][n+1] >= (r_p - min_water)):
                values[prec_mod]['L_65_'+impact][n+1] = r_p - min_water
                values[prec_mod]['m_65_'+impact+'_A'][n+1] = values[prec_mod]['m_65_'+impact+'_A'][n] + mass_increase(m_dot_I, model = impact, wi = w_i, fs = f_s[impact]) + photo_destruction(constants_and_rates['adenine']['M_uv_dot'], values[prec_mod]['m_65_'+impact+'_A'][n], constants_and_rates['adenine']['rho'], constants_and_rates['adenine']['d'])
                if impact == 'Met':
                    pause_Met[prec_mod] += 1
            else:
                values[prec_mod]['m_65_'+impact+'_A'][n+1] = values[prec_mod]['m_65_'+impact+'_A'][n] + mass_increase(m_dot_I, model = impact, wi = w_i, fs = f_s[impact]) + nucleobase_outflow(n, pause_Met[prec_mod], constants_and_rates['adenine']['k'], model = impact) + decomposition(constants_and_rates['adenine']['k'], values[prec_mod]['m_65_'+impact+'_A'][n]) + seepage(values[prec_mod]['m_65_'+impact+'_A'][n], values[prec_mod]['m_65_'+impact][n])
            if values[prec_mod]['m_65_'+impact+'_A'][n+1] < 0:
                values[prec_mod]['m_65_'+impact+'_A'][n+1] = 0
            values[prec_mod]['m_65_'+impact][n+1] = water_mass(prec_mod, impact, n+1)
            # update concentration
            values[prec_mod]['C_65_'+impact][n+1] = values[prec_mod]['m_65_'+impact+'_A'][n+1] / values[prec_mod]['m_65_'+impact][n+1]    
        # then the aqueous equations
        for name in names:
            # we only work with masses in the pond, not from Met or IDP here
            if name[0] == 'm' and any([x in name for x in ['Met', 'IDP']]) == False:
                #print(name)
                spec_name = name.split('_')[2] # the format is m_65_spec_etc
                spec_mdot = get_mdot(spec_name, name.split('_')[-1]) # type is the last part of the srting
                
                if (values[prec_mod]['L_65_IDP'][n+1] >= (r_p - min_water)):
                    values[prec_mod]['L_65_IDP'][n+1] = r_p - min_water
                    ...
                    values[prec_mod][name][n+1] = values[prec_mod][name][n] + mass_increase(spec_mdot) + decomposition(constants_and_rates[spec_name]['k'], values[prec_mod][name][n]) + seepage(values[prec_mod][name][n], values[prec_mod]['m_65_IDP'][n])
                if values[prec_mod][name][n+1] < 0:
                    values[prec_mod][name][n+1] = 0
                # update concentrations (biomolecule mass / water mass)
                conc_name = 'C' + name[1:]
                values[prec_mod][conc_name][n+1] = values[prec_mod][name][n+1] / values[prec_mod]['m_65_IDP'][n+1]
...
p1 = ax1.fill_between(t, values['IDN']['C_65_adenine_red']*Adenine_lower*1e6/constants_and_rates['adenine']['mu'], values['IDN']['C_65_adenine_red']*Adenine_upper*1e6/constants_and_rates['adenine']['mu'], linestyle='-', color='#34b33a', lw=3.5, label=r'$\it{in}$ $\it{situ}$ Production - Early Hadean (reducing)', alpha=.60)
         
p2, = ax1.plot(t, values['IDN']['C_65_Met']*1e6/constants_and_rates['adenine']['mu'], linestyle='-', color='#9d620c', lw=3.5, label='Meteorites - Intermediate Env.')
         
p3, = ax1.plot(t, values['IDN']['C_65_IDP']*1e6/constants_and_rates['adenine']['mu'], linestyle='-', color='#9f53e6', lw=3.5, label='IDPs - Intermediate Env.')  
        
p4 = ax1.fill_between(t,values['IDN']['C_65_adenine_ox']*Adenine_lower*1e6/constants_and_rates['adenine']['mu'], values['IDN']['C_65_adenine_ox']*Adenine_upper*1e6/constants_and_rates['adenine']['mu'], linestyle='--', color='#095909', lw=3.5, label=r'$\it{in}$ $\it{situ}$ Production - Late Hadean (oxidizing)', alpha=.60)
                        
p5, = ax1.plot(t, values['COL']['C_65_Met']*1e6/constants_and_rates['adenine']['mu'], linestyle='--', color='#9d620c', lw=3.5, label='Meteorites - Wet Env.')      
               
#p6, = ax1.plot(t, C_IDN_65_Adenine_haze5*1e6/mu_Adenine, linestyle='-', color='#a30313', lw=3.5, label='Organic Hazes (5% methane)') 
  
p7 = ax1.fill_between(t, values['IDN']['C_65_adenine_haze05_min']*1e6/constants_and_rates['adenine']['mu'], values['IDN']['C_65_adenine_haze05_max']*1e6/constants_and_rates['adenine']['mu'], linestyle='-', hatch = '/',lw=3.5, color="#999999", label='Organic Hazes (0.5% methane)',alpha=0.4)                       
...
ax2.plot(t, values['IDN']['C_65_uracil_haze05_max']*1e6/constants_and_rates['uracil']['mu'], linestyle='-', color='#AA3377', lw=3.5, label='Uracil')     
ax2.plot(t, values['IDN']['C_65_hypoxanthine_haze05_max']*1e6/constants_and_rates['hypoxanthine']['mu'], linestyle='--', color='#66CCEE', lw=3.5, label='Hypoxanthine')  
ax2.plot(t, values['IDN']['C_65_xanthine_haze05_max']*1e6/constants_and_rates['xanthine']['mu'], linestyle='-', color='#EE6677', lw=3.5, label='Xanthine*')   
ax2.plot(t, values['IDN']['C_65_thymine_haze05_max']*1e6/constants_and_rates['thymine']['mu'], linestyle='--', color='#BBBBBB', lw=3.5, label='Thymine')     
ax2.plot(t, values['IDN']['C_65_guanine_haze05_max']*1e6/constants_and_rates['guanine']['mu'], linestyle='-', color='#4477AA', lw=3.5, label='Guanine*')      
ax2.plot(t, values['IDN']['C_65_cytosine_haze05_max']*1e6/constants_and_rates['cytosine']['mu'], linestyle='--', color='#CCBB44', lw=3.5, label='Cytosine')    
...
plt.tight_layout()
plt.savefig('Nucleobase_Comparison_Multiple_Sources_2023_own.png',dpi=300)
